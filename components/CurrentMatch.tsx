...
export default function CurrentMatch({ match, onPickWinner, onStartNext }: CurrentMatchProps) {
  if (!match) {
    return (
      <div className="bg-white p-6 rounded-lg shadow text-center text-gray-500">
        No active match. Shuffle and split players to start!
      </div>
    );
  }
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow">
      <h3 className="text-2xl font-bold text-center mb-6">Current Match</h3>
      
      <div className="grid grid-cols-3 gap-4 items-center mb-6">
        <div className="text-center">
          <div className="bg-blue-100 p-4 rounded-lg">
            <h4 className="text-xl font-bold text-blue-700 mb-2">Team A</h4>
            <ul className="space-y-1">
              {match.teamA.players.map((player) => (
                <li key={player.id} className="text-sm">{player.name}</li>
              ))}
            </ul>
          </div>
        </div>
        ...
        <div className="text-center">
          <div className="bg-red-100 p-4 rounded-lg">
            <h4 className="text-xl font-bold text-red-700 mb-2">Team B</h4>
            <ul className="space-y-1">
